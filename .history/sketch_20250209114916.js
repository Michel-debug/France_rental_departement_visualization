...
// =========== 新增：地图过渡动画所需的变量 ===========
let FRANCE_BBOX = { minLon: -5, maxLon: 10, minLat: 41, maxLat: 51 }; // 整个法国简略范围
let deptBbox = null;       // 选中部门的 bbox
let currentBbox = { ...FRANCE_BBOX }; // 当前使用的 bbox，会在动画中插值
let isTransitioning = false;
let fromBbox = null;
let toBbox = null;
let transitionStartFrame = 0;
let transitionDuration = 60; // 过渡动画时长(帧)

// 加载和 setup
function setup() {
  createCanvas(1400, 1200);
  noLoop(); // 数据加载完成后再 redraw()
...
// -------------- 辅助：在某个 bbox 中绘制一个 ring --------------
function drawSinglePolygonInBbox(ring, bbox, fillCol = '#ADD8E6', highlight = false) {
  beginShape();
  for (let coord of ring) {
    let lon = coord[0];
    let lat = coord[1];
...
// --------------- 部门内：绘制学校 ---------------
function drawDeptEtablissements(deptCode, bbox) {
  let bestDist = Infinity;
  let flickerScale = 1 + 0.1 * sin(frameCount * 0.1); // 小幅度呼吸动效
...
// --------------- 部门内：绘制自行车站点 ---------------
function drawDeptCyclingStations(deptCode, bbox) {
  let bestDist = Infinity;
  // 小幅alpha闪烁
  ...
  for (let station of cyclingStationsData) {
    if (station.departement_code === deptCode) {
      ...
      // capacity -> alpha (叠加一个闪烁)
      let baseAlpha = map(capacite, minCap, maxCap, 80, 255);
      let alphaVal = lerp(baseAlpha, flickerAlpha, 0.5); // 混合一点闪烁
...
// ---------- 辅助：单个 Polygon 的点内判断 ----------
function checkPolygon(x, y, coordinates) {
  // 在全国模式下，用 FRANCE_BBOX (或 currentBbox) 做投影
  // 这里用 currentBbox 实现“地图缩放中的命中检测”
  const screenPoly = coordinates[0].map(coord => {
    const px = map(coord[0], currentBbox.minLon, currentBbox.maxLon, 50, width - 50);
    const py = map(coord[1], currentBbox.minLat, currentBbox.maxLat, height - 50, 50);
    return { x: px, y: py };
  });
  return isPointInPolygon(x, y, screenPoly);
}
...
function drawDepartments() {
  ...
  for (let i = 0; i < features.length; i++) {
    ...
    if (inside) {
      hoveredDept = deptCode;
      // 你也可以让颜色更亮点 highlightCol = color(red(fillCol)+30, ...);
    }

    fill(highlightCol);
    stroke(100);

    // 绘制 Polygon 或 MultiPolygon
    if (geometryType === "Polygon") {
      for (let ring of coordinates) {
        beginShape();
        for (let coord of ring) {
...
function draw() {
  ...
  if (!selectedDept) {
    ...
    // 部门内 icon
    drawDeptCyclingStations(selectedDept, currentBbox);
    drawDeptEtablissements(selectedDept, currentBbox);
...
function checkDataLoaded() {
  if (
    ...
    etablissementsData.length > 0 &&
    cyclingStationsData.length > 0 &&
    bikeIcon && privateIcon && publicIcon
  ) {
    dataLoaded = true;
    redraw(); // 数据准备就绪后，触发一次初始绘制
  }
