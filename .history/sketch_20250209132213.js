...
// 生成渐变颜色数组
const DEPARTEMENT_COLORS = Array.from({ length: FRENCH_DEPARTEMENT_COUNT }, (_, i) => {
  const hue = (i * 300 / FRENCH_DEPARTEMENT_COUNT) % 360; // 控制色相范围(0-300)
  const saturation = 70 + 30 * Math.sin(i * 0.3);         // 饱和度波浪变化
  const lightness = 45 + 25 * Math.cos(i * 0.2);          // 明度波浪变化
