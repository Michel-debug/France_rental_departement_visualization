...
function drawBoxPlot(deptCode) {
  ...
  const plotY = 50;
  const plotWidth = 320;
  const plotHeight = 300; // 增大绘图区域高度

  // 定义动态放大比例（扩大数据范围 20%）
  const expansionFactor = 0.2; // 放大比例 20%
  const expandedMin = data.min - (data.max - data.min) * expansionFactor; // 最小值向下扩展
  const expandedMax = data.max + (data.max - data.min) * expansionFactor; // 最大值向上扩展
