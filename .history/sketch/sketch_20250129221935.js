let regionsGeoJSON; // GeoJSON 文件
let rentalData = {}; // 租金数据（从 JSON 文件加载）

function preload() {
  // 加载 GeoJSON 文件和租金数据文件
  regionsGeoJSON = loadJSON("path/to/regions.geojson");
  let rentalDataArray = loadJSON("path/to/rental_data.json");

  // 处理租金数据，将其转换为 {region_code: 数据对象} 格式
  rentalDataArray.then(data => {
    data.forEach(item => {
