...
function drawBoxPlot(deptCode) {
  ...
  const actualMax = Math.max(data.max, MAX_PRICE);
  const actualMin = Math.min(data.min, MIN_PRICE);
  ...
  const priceToY = price => {
    const bufferRange = (actualMax - actualMin) * 0.1;
    return map(price, 
      actualMin - bufferRange, 
      actualMax + bufferRange,
      plotY + plotHeight - 15,  // 下边距
      plotY + 15                // 上边距
    );
  };

  // 计算各要素坐标（增加边界保护）
  const minY = constrain(priceToY(data.min), plotY + 15, plotY + plotHeight - 15);
  const q1Y = constrain(priceToY(data.q1), plotY + 15, plotY + plotHeight - 15);
  const medianY = constrain(priceToY(data.median), plotY + 15, plotY + plotHeight - 15);
  const q3Y = constrain(priceToY(data.q3), plotY + 15, plotY + plotHeight - 15);
  const maxY = constrain(priceToY(data.max), plotY + 15, plotY + plotHeight - 15);

  // 绘制箱线图元素
  const boxWidth = constrain(map(iqrHeight, 0, plotHeight, 60, 100), 60, 100);
  const boxX = plotX + (plotWidth - boxWidth)/2; // 水平居中
