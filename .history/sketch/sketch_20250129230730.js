...
function setup() {
  ...
  loadJSON('data/departement_rental_data.json', function(data) {
    if (Array.isArray(data)) {
      ...
      data.forEach(item => {
        rentalData[item.departements_code] = item; // 转换为字典格式
      });
  ...
  loadJSON('data/departements.geojson', function(data) {
    departementssGeoJSON = data;
    console.log("✅ GeoJSON 数据加载成功:", departementssGeoJSON);
    checkDataLoaded();
  });
}

function drawdepartementss() {
  let features = departementssGeoJSON.features; // 获取所有区域

  // 遍历每个区域
  for (let i = 0; i < features.length; i++) {
    let departements = features[i];
    let coordinates = departements.geometry.coordinates;
    let departementsCode = departements.properties.code; // 区域代码

    // 获取该区域对应的租金数据
    let rentalInfo = rentalData[departementsCode];
    ...
    textAlign(CENTER);
    if (rentalInfo) {
      text(`${departements.properties.nom}\n${medianPrice.toFixed(1)} €/m²`, labelX, labelY);
    } else {
      text(departements.properties.nom, labelX, labelY);
    }
  }
}
...
function draw() {
  ...
  if (dataLoaded) {
    ...
    drawdepartementss(); // **只有数据加载完成后才执行绘制**
  } else {
    console.log("⌛ 数据加载中...");
    textSize(20);
...
function checkDataLoaded() {
  if (departementssGeoJSON && rentalDataArray.length > 0) {
