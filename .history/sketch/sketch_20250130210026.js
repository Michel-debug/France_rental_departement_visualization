...
function drawBoxPlot(deptCode) {
  ...
  // 计算各要素坐标（增加边界保护）
  const minY = constrain(priceToY(data.min), plotY + 30, plotY + plotHeight - 30);
  const q1Y = constrain(priceToY(data.q1), plotY + 30, plotY + plotHeight - 30);
  const medianY = constrain(priceToY(data.median), plotY + 30, plotY + plotHeight - 30);
  const q3Y = constrain(priceToY(data.q3), plotY + 30, plotY + plotHeight - 30);
  const maxY = constrain(priceToY(data.max), plotY + 30, plotY + plotHeight - 30);
