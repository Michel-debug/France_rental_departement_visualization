...
function setup() {
  ...
  loadJSON("data/departements.geojson", function(geoData) {
    loadJSON("data/region_rental_data.json", function(rentalArray) {
...
function drawLegend() {
  ...
  text(`${MAX_PRICE} €/m²`, legendX + legendWidth + 5, legendY);
  text(`${MIN_PRICE} €/m²`, legendX + legendWidth + 5, legendY + legendHeight);

  // 你也可以在中间加些刻度，或者额外写个标题
  text("Rent legend", legendX, legendY 15);
}
