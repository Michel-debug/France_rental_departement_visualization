...
const PIE_COLORS = Array.from({ length: FRENCH_DEPARTEMENT_COUNT }, (_, i) => {
  ...
  if (t < 0.33) {
    ...
    // 蓝： h=240, s=80, l=50
    // 白： s=0 或 l=100
    let h = lerpColor(240, 0, subT); // hue 不重要, s=0 相当于灰度，h 无意义
    let s = lerpColor(80, 0, subT);  // s:80 -> 0
    let l = lerpColor(50, 100, subT); // l:50 -> 100
    ...
    // 红： h=0, s=80, l=50
    // 白： s=0, l=100
    let h = lerpColor(0, 0, subT); // 0 => 0
    let s = lerpColor(0, 80, subT);
    let l = lerpColor(100, 50, subT);
    colorHex = hslToHex(h, s, l);
  }
