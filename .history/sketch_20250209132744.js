...
// 法国有约 101 个 département（含一些海外省等）
const FRENCH_DEPARTEMENT_COUNT = 101;

// 线性插值函数（如果你使用 p5.js，可以用自带的 lerp()）
function lerp(start, end, amt) {
  return start + (end - start) * amt;
}

// 生成一个从“淡蓝色” -> “红色”的 HSL 渐变数组
const PIE_COLORS = Array.from({ length: FRENCH_DEPARTEMENT_COUNT }, (_, i) => {
  // t 在 0~1 之间
  let t = i / (FRENCH_DEPARTEMENT_COUNT - 1);

  // ========== 起始/结束 HSL 可根据需求微调 ==========
  // 淡蓝 (H=210, S=40, L=80) → 红 (H=0, S=80, L=50)
  let hueStart = 210, hueEnd = 0;
  let satStart = 40, satEnd = 80;
  let litStart = 80, litEnd = 50;

  // 做线性插值
  let h = lerp(hueStart, hueEnd, t);
  let s = lerp(satStart, satEnd, t);
  let l = lerp(litStart, litEnd, t);
