...
function drawDeptEtablissements(deptCode) {
  ...
  for (let e of etablissementsData) {
    ...
    if (e.departement_code === deptCode) {
      ...
      // 根据 secteur 选择颜色
      let secteur = e["secteur d'établissement"] || e["secteur d'\u00e9tablissement"] || "public";
      // 你也可以统一改成 e.secteur
      let iconImg = (secteur === "public") ? publicSchoolIcon : privateSchoolIcon;
...
Secteur: ${e["secteur d'\u00e9tablissement"]}`;
  ...
  textSize(12);
  textAlign(LEFT, TOP);
  text(infoText, tooltipX + pad, tooltipY + pad);
}
